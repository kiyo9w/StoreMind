...
class SourceTag extends InlineMd {
  ...
  InlineSpan span(
    ...
    // Extract all numbers from the content string (e.g., "[1] [2]" -> "1,2")
    final citationRegex = RegExp(r'\[(\d+)\]');
    final matches = citationRegex.allMatches(content);
    final indices = matches.map((m) => m.group(1)!).join(',');
      ...
      child: Padding(
        ...
        child:
            config.sourceTagBuilder?.call(
              context,
              indices, // Pass comma-separated indices
              const TextStyle(),
            ) ??
            SizedBox(
              width: 20,
              ...
              child: Material(
                ...
                child: FittedBox(
                  ...
                  child: Text(indices, textDirection: config.textDirection),
                ),
              ),
            ),
      ),
