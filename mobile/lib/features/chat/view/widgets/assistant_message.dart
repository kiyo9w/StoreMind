...
// Note: AgentActivityIndicator is still available for future use if needed
import 'package:insider/features/chat/view/widgets/agent_activity_indicator.dart';
...
class AssistantMessage extends StatefulWidget {
  ...
  final VoidCallback? onPlanUpdateTap;
  ...
  Widget _buildAnswerContent(bool hasAgentSteps) {
    return Column(
      ...
      children: [
        ...
        // Beautiful thinking/searching dropdown - the primary indicator during streaming
        if (hasAgentSteps)
          ExpertSearchPreview(
            isDark: widget.isDark,
        ...
        if (widget.message.isStreaming && widget.message.content.isEmpty) ...[
          ...
          // Always use MarkdownText for beautiful rendering with citations
          // This ensures consistent formatting whether streaming or not
          Padding(
        ...
        // Plan update notification (Manager mode)
        if (widget.message.planUpdated && !widget.message.isStreaming)
          PlanUpdateBadge(
            actionModified: widget.message.actionModified,
            onTap: widget.onPlanUpdateTap,
          ),
            ...
            child: IntrinsicHeight(
              child: Row(
                ...
                children: [
                  ...
                  // Content - List of steps grouped by agent
                  Expanded(
                    child: _AgentGroupedSteps(
                      agentSteps: widget.agentSteps,
                      isDark: widget.isDark,
                      isStreaming: true,
                    ),
            ...
            child: Row(
              ...
              children: [
                ...
                Text(
                  step.toolName!,
                  style: TextStyle(
                    color: textPrimary, // Use textPrimary from parent scope
                    fontSize: 13,
                    fontWeight: FontWeight.w500,
                  ),
                ),
...
/// Groups agent steps by agent name and renders each group with a header.
/// This ensures the thinking dropdown shows clear separation between
/// Orchestrator, Stocker, Planner, and Reviser contributions.
class _AgentGroupedSteps extends StatelessWidget {
  const _AgentGroupedSteps({
    required this.agentSteps,
    required this.isDark,
  ...
  /// Get an icon for a given agent name
  static IconData _agentIcon(String? name) => switch (name) {
        'Orchestrator' => Icons.hub_outlined,
        'Stocker' => Icons.inventory_2_outlined,
        'Planner' => Icons.event_note_outlined,
        'Reviser' => Icons.fact_check_outlined,
        _ => Icons.smart_toy_outlined,
      };

  /// Get a color for a given agent name
  static Color _agentColor(String? name) => switch (name) {
        'Orchestrator' => const Color(0xFF6E8EF7),
        'Stocker' => const Color(0xFF4ECDC4),
        'Planner' => const Color(0xFFFF9F43),
        'Reviser' => const Color(0xFFAB7DF6),
        _ => Colors.grey,
      };
  ...
  Widget build(BuildContext context) {
    ...
    final visibleSteps =
        agentSteps.where((s) => s.status != AgentStepStatus.pending).toList();
    if (visibleSteps.isEmpty) return const SizedBox.shrink();

    // Group consecutive steps by agent
    final groups = <({String? name, String? role, List<AgentStep> steps})>[];
    for (final step in visibleSteps) {
      if (groups.isEmpty || groups.last.name != step.agentName) {
        groups.add((
          name: step.agentName,
          role: step.agentRole,
          steps: [step],
    ...
    return Column(
      ...
      children: [
        for (var i = 0; i < groups.length; i++) ...[
          ...
          if (groups[i].name != null)
            Padding(
              ...
              child: Row(
                children: [
                  Icon(
                    _agentIcon(groups[i].name),
                    size: 13,
                    color: _agentColor(groups[i].name),
                  ),
          ...
          // Steps for this agent
          for (var j = 0; j < groups[i].steps.length; j++) ...[
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          Padding(
            ...
            child: Row(
              children: [
                ...
                // Count unique agents
                Builder(builder: (_) {
                  final uniqueAgents = agentSteps
                      .map((s) => s.agentName)
                      .where((n) => n != null)
                      .toSet();
                  return Text(
                    uniqueAgents.length > 1
                        ? '${uniqueAgents.length} agents • ${agentSteps.length} steps'
          ...
          const SizedBox(height: 16),
          // Timeline content — grouped by agent
          Expanded(
            child: SingleChildScrollView(
              padding: const EdgeInsets.fromLTRB(16, 0, 16, 24),
              child: _AgentGroupedTimeline(
...
/// Groups agent steps by agent name and renders timeline with agent headers
class _AgentGroupedTimeline extends StatelessWidget {
  const _AgentGroupedTimeline({
    required this.agentSteps,
    required this.isDark,
  });
  ...
  Widget build(BuildContext context) {
    ...
    for (var gi = 0; gi < groups.length; gi++) {
      ...
      // Agent header separator
      if (group.name != null) {
        final color = _AgentGroupedSteps._agentColor(group.name);
        flatItems.add(
          Padding(
            padding: EdgeInsets.only(top: gi > 0 ? 8 : 0, bottom: 8),
            child: Row(
              children: [
                Container(
                  width: 20,
                ...
                const SizedBox(width: 8),
                Icon(
                  _AgentGroupedSteps._agentIcon(group.name),
                  size: 14,
                  color: color,
                ),
                const SizedBox(width: 5),
      ...
      for (var si = 0; si < group.steps.length; si++) {
        final isLast = isLastGroup && si == group.steps.length - 1;
        flatItems.add(_TimelineStepItem(
          step: group.steps[si],
          isDark: isDark,
          isLast: isLast,
          accentColor: group.name != null
              ? _AgentGroupedSteps._agentColor(group.name)
              : null,
        ));
      }
    }

    return Column(children: flatItems);
  }
}
...
class _TimelineStepItem extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final textSecondary = isDark
        ? DesignSystem.textSecondaryDark
        : DesignSystem.textSecondaryLight;
    final dotColor = accentColor?.withOpacity(0.5) ?? textSecondary.withOpacity(0.4);
    final lineColor = accentColor?.withOpacity(0.15) ?? textSecondary.withOpacity(0.2);
